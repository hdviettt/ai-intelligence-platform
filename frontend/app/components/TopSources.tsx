...
export function TopSources({ sources }: { sources: TopSource[] }) {
  ...
  return (
    <div className="rounded-xl border border-md-outline-variant bg-md-surface-container-low p-5">
      <span className="md-title-small text-md-on-surface">Top sources</span>
        ...
        {sources.map((s) => {
          ...
          return (
            <div key={s.source} className="flex items-center gap-2 text-sm">
              <span className="w-28 shrink-0 truncate text-md-on-surface-variant" title={s.source}>
                {s.source}
              </span>
              <div className="relative h-5 flex-1 overflow-hidden rounded-full bg-md-surface-container">
                <div
                  className={`h-full rounded-full transition-[width] duration-500 ease-md-emphasized ${style.dot}`}
                  style={{ width: `${Math.max(2, pct)}%`, opacity: 0.85 }}
                />
              </div>
              <span className="w-12 text-right tabular-nums text-md-on-surface">
